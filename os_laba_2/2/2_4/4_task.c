...
void situation_c() {
    ...
    if (pid == 0) {
        ...
        system("ps -o pid,ppid,state,command -C father.out > situation_c.log");
        sleep(0); // Фиксируем зомби
    }
}
...
int main() {
    ...
    sleep(10);
    printf("Results saved to situation_c.log and final_report.log\n");
    system("ps -o pid,ppid,state,command -C father.out > final_report.log");
    return 0;
}
